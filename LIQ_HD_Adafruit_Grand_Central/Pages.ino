...
void pause_page()
{
  while (display_page == "pause") {
    ...
    if (ts.touched()) {
      ...
      if (p.x > 65 && p.x < 175 && p.y > 230 && p.y < 270) { //Resume button coordiantes, Unpause --> mount SD card and continue recording to the same file --> go to recording_page (display_page = 2)
        p = {}; //debounce touchscreen
